...
func TestSensitiveValidateWithWild(t *testing.T) {
	filter := New()
	filter.AddWord("有一个东西")
	filter.AddWord("一个东西")
	filter.AddWord("一个")
	...
	testcases := []struct {
		...
		{"我有一个东西", false, "有一个东西", []string{"有一个东西", "一个"}, false, "一个东西"},
		{"一个东西", false, "一个", []string{"个东", "一个"}, false, "一个东西"},
		{"卖国贼", false, "卖*贼", []string{"卖*贼"}, true, ""},
		{"卖*贼", false, "卖*贼", []string{"卖*贼"}, true, ""},
		{"一个丑陋的卖*贼", false, "一个", []string{"一个", "卖*贼"}, true, ""},
	}

	for _, tc := range testcases {
		if pass, first := filter.ValidateWithWild(tc.Text, '*'); pass != tc.ExpectPass || first != tc.ExpectFirst {
			t.Errorf("validate %s, got %v, %s, expect %v, %s", tc.Text, pass, first, tc.ExpectPass, tc.ExpectFirst)
		}

		filter.DelWord(tc.DelWords...)

		if pass, first := filter.ValidateWithWild(tc.Text, '*'); pass != tc.ThenExpectPass || first != tc.ThenExpectFirst {
			t.Errorf("after del, validate %s, got %v, %s, expect %v, %s", tc.Text, pass, first, tc.ThenExpectPass, tc.ThenExpectFirst)
		}
...
func TestReplaceWithWild(t *testing.T) {
	filter := New()
	filter.AddWord("有一个东西")
	filter.AddWord("一个东西")
	filter.AddWord("一个")
	...
	for _, tc := range testcases {
		if got := filter.ReplaceWithWild(tc.Text, '*', '*'); got != tc.Expect {
			t.Errorf("replace %s, got %s, expect %s", tc.Text, got, tc.Expect)
		}

		filter.DelWord(tc.DelWords...)

		if got := filter.ReplaceWithWild(tc.Text, '*', '*'); got != tc.ThenExpect {
