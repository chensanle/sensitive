...
func (filter *Filter) ReplaceWithWild(text string, repl, wild rune) string {
	return filter.trie.ReplaceWithWildcard(text, repl, wild)
}
...
func (filter *Filter) ValidateWithWild(text string, wild rune) (bool, string) {
	text = filter.RemoveNoise(text)
	return filter.trie.ValidateWithWildcard(text, wild)
}
